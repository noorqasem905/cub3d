...
/*   Updated: 2025/07/13 21:05:38 by nqasem           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    cub3d->map.visited = alloc_2d_array(cub3d->map.map_height, cub3d->map.map_width);
    if (!cub3d->map.visited)
    {
        handle_error(ERO_MALLOC);
        return (-1);
    }
    if (!flood_fill(&cub3d->map, cub3d->player.map_x, cub3d->player.map_y, cub3d))
    {
